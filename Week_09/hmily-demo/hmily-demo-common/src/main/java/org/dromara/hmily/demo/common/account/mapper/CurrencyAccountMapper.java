...
public interface CurrencyAccountMapper {
     ...
     * @return the int
     */
    @Update("update account set unused_amount = unused_amount + #{amount}," +
            "update_time = now()" +
    ...
    int confirmDecrease(AccountDTO accountDTO);
    ...
    @Update("update account set balance = balance + #{amount}," +
            " freeze_amount= freeze_amount -  #{amount} " +
            " where user_id =#{userId}  and freeze_amount >0")
    int cancelDecrease(AccountDTO accountDTO);
     ...
     */
    @Update("update account set " +
            " amount= amount + #{amount}, unused_amount = unused_amount - #{amount}" +
            " where user_id =#{userId}  and unused_amount >0 ")
    int confirmIncrease(AccountDTO accountDTO);
    ...
    @Update("update account set unused_amount = unused_amount - #{amount}" +
            " where user_id =#{userId}  and unused_amount >0")
    int cancelIncrease(AccountDTO accountDTO);
